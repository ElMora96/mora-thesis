...
class CryptoNet(object):
    ...
    def build_model(self):
        ...
        self.msg = tf.compat.v1.placeholder("float", [None, self.msg_len])
        self.key = tf.compat.v1.placeholder("float", [None, self.key_len])
    ...
    def train(self):
        ...
        #DEBUG
        #print("(debug) Tipo loss", type(self.loss_bob))
        # Build the optimizers
        self.bob_optimizer = tf.compat.v1.train.AdamOptimizer(self.learning_rate).minimize(
            loss = self.loss_bob, var_list=self.alice_or_bob_vars)
        self.eve_optimizer = tf.compat.v1.train.AdamOptimizer(self.learning_rate).minimize(
            loss = self.decrypt_err_eve, var_list=self.eve_vars)
