...
\end{align}
where $w_i$ is the weight at the input $x_i$. Such weights, according to the biological model, need to be adjustable. The output of the cell, instead, is denoted by $y$, that is a nonlinear activation function $f_N$ (see Subsection \ref{percact} below) of the node output $z$. \\
...
Such training algorithms revealed themselves to be challenging, especially from the introduction of the so called Multi-Layer Perceptron (MLP). In fact, already in the late 1960s, limitations of single-layer architectures were shown (see \cite{minpapert}), together with the possibility of solving more complicated problems using MLP's. However, finding efficient algorithms to adjust the weights of such nets was problematic and took several years. Some examples of solutions to these problems are presented in the following sections. All of such algorithms are thought to work in discrete time, while taking into account partial derivatives of the quantities of interest. Please read appendix \ref{fdmappendix} for further detail about Finite Difference Methods (FDMs).
...
\begin{equation}
\frac{1}{\lambda_{\text{max}}} > \mu > 0,
\end{equation}
where $\lambda_{\text{max}}$ is the maximum eigenvalue of the matrix $R$ of equation \eqref{expectdesign}.
\end{enumerate}
...
%\KwResult{Write here the result }
 ...
 \Return{$\theta_t$}
 \caption{\textit{Adam} stochastic optimization algorithm. The elementwise square $g_t \cdot g_t$ is denoted $g_t^{2}$.}
\end{algorithm}
...
However, several researchers have combined neural network with classical cryptography for the multivariate structural and nondirectional features of neural network. More precisely, the concept of cryptography based on NNs was firstly introduced in \cite{lauria}.Then branches of applications and related works of cryptography with different NN models were proposed subsequently. In particular, it came out that network stochastic synchronization provides a possibility for mutual learning between neural networks, allowing researchers to build secure communication channels. \\
...
In this chapter we discuss some of the first neural cryptosystems.
\newpage
...
\begin{align*}
    ...
    e_0, e_1 &= \text{random digits} \\
  	e_2 &= \text{sign bit}.
\end{align*}
The total number of layers is denoted by $L$, while $n_i$ and $n_k$ are the number of units in layers $i$ and $k$, respectively. Recall that these values are determined by the execution of the genetic algorithm. \\
...
by a suitable divided difference of function values at the chosen grid points. Such approximations of derivatives by difference formulas can be generated in various ways; in case they are generated by a Taylor expansion, the approach is generally known in literature as finite difference method. \\
...
\end{equation}
Here $h = \frac{1}{N}$, $N \in \mathbb{N}\setminus \{0\}$, is the mesh size of the grid. \\
...
\end{align}
For any $N\in \mathbb{N}$, $N > 1$ this linear system has a unique solution $u_{i,j}$. Under certain smoothness assumptions on the desired solution $u$ of the original problem one has $u_{i,j} \approx u(x_{i,j})$.
...
of intersection obtained when one translates the coordinate axes through consecutive equidistant steps with step size $h=\frac{1}{N}$. Here $N\in \mathbb{N} \setminus \{0\}$ denotes the number of shifted grid lines in each coordinate direction. \\
