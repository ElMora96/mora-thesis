...
\begin{align}
\text{if} \quad \left(O^{S/R}y_k^{S/R}>0\right) \quad &\text{then} \quad w_{k,j}^{S/R} = w_{k,j}^{S/R} -O^{S/R} x_{k,j}; \\
\text{if} \quad \left(|w_{k,j}^{S/R}|>L\right) \quad &\text{then} \quad w_{k,j}^{S/R} = sgn\left(w_{k,j}^{S/R}\right)L.
\end{align}
